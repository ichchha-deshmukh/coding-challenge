...
const CHARACTER_MAP = ["A", "B", "C"];
const DIVISOR = CHARACTER_MAP.length;

exports.translate = function (number) {
  let input = number;
  const FINAL_LETTERS = [];
  while (input > 0) {
    FINAL_LETTERS.push(CHARACTER_MAP[input % DIVISOR]);
    input = Math.floor(input / DIVISOR);
  }

  return FINAL_LETTERS.reverse().join("");
};
